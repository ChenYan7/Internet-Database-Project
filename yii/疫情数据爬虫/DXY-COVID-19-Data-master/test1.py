...
db = pymysql.connect(host="localhost", user="root", password="", database="yii", charset="utf8")
...
class Listener:
    ...
    def mysql_dumper(self, table, da):  # current_data是一个json文件，字典套列表套字典
        if table == 'DXYNews':
            ...
            select_sql = "select * from dxyarea where countryName=%s"
            for item in da['results']:
                # 中国表的更新
                if (type(item["cities"]).__name__=='list'):#中国表的更新
                    select_sql1 = "select * from china where cityName=%s"
                    for city in item['cities']:
                        self.cursor.execute(select_sql1,city["cityName"])
                        data = self.cursor.fetchall()
                        if len(data)!=0:
                            if  data[0][1]!=city["currentConfirmedCount"] or data[0][2]!=city["confirmedCount"]:
                                value1=(city["currentConfirmedCount"],city["confirmedCount"],city["suspectedCount"],
                                city["curedCount"],city["deadCount"],city["cityName"])
                                sql6="Call updateChinaProc(%s,%s,%s,%s,%s,'%s')"%value1
                                self.cursor.execute(sql6)
                                self.db.commit()
                    item["cities"]="见中国表"
                # 世界表的更新
                self.cursor.execute(select_sql, item["countryName"])
                data = self.cursor.fetchall()
                if len(data) != 0:#之前出现过就更新
                        newvalue = ( item["countryName"],item["currentConfirmedCount"], item["confirmedCount"], item["suspectedCount"],
                                  item["curedCount"], item["deadCount"])
                        sql4="Call updateProc('%s',%s,%s,%s,%s,%s)"%newvalue
                        test="Call updateProc(1,2,3,4,5,6)"
                        self.cursor.execute(sql4)
                        self.db.commit()
                else:#之前没有出现过就插入
                    sql2="insert into dxyarea values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"

                    value2 = (
                          self.cnt, item["continentName"], item["continentEnglishName"], item["countryName"],
                          item["countryEnglishName"], item["countryFullName"], item["provinceName"],
                          item["provinceEnglishName"],
                          item["provinceShortName"],  item["currentConfirmedCount"],  item["confirmedCount"],
                          item["suspectedCount"],
                          item["curedCount"],  item["deadCount"], item["comment"], item["cities"],
                          item["updateTime"])
                    self.cnt=self.cnt+1;
